...
import CustomerLifecycleBar from '@/components/customers/CustomerLifecycleBar';
import CustomerQuickFacts from '@/components/customers/CustomerQuickFacts';
import CustomerRecordNavigator from '@/components/customers/CustomerRecordNavigator';
...
export interface CustomerRecordSummaryCardProps {
  customer: Customer;
...
export default function CustomerRecordSummaryCard({
  customer,
  formData,
  logoPanel,
  actions,
  ...
  className = '',
}: CustomerRecordSummaryCardProps) {
  ...
  if (isDraft) {
    ...
    subtitleParts.push(formatCustomerCode(customer.customer_code));
  }
  ...
  const title = formData.legal_name?.trim() || customer.legal_name || 'New customer';
      ...
      <div className="flex flex-wrap items-stretch gap-3">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <div className="mt-2.5 flex w-full items-start justify-between gap-x-6 gap-y-3 pt-2.5">
            ...
            <CustomerQuickFacts formData={formData} />
          </div>
        </div>
      </div>
