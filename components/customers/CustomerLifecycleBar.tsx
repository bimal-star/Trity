...
const CUSTOMER_LIFECYCLE_STAGES: { id: CustomerStatus; label: string }[] = [
...
export interface CustomerLifecycleBarProps {
...
export default function CustomerLifecycleBar({
  status,
  isArchived = false,
  disabled = false,
  onStatusChange,
  ...
  const normalized = String(status) as CustomerStatus;
  const currentIndex = CUSTOMER_LIFECYCLE_STAGES.findIndex((s) => s.id === normalized);
  ...
  return (
    ...
    >
      {CUSTOMER_LIFECYCLE_STAGES.map((stage, i) => {
        ...
        return (
          <div key={stage.id} className="flex items-center">
            ...
            {i < CUSTOMER_LIFECYCLE_STAGES.length - 1 ? (
              <span
