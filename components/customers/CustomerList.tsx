...
const VIS_FILTERS: { value: CustomerRecordVisibility; label: string }[] = [
...
interface CustomerListProps {
  ...
  recordVisibility: CustomerRecordVisibility;
  onRecordVisibilityChange: (value: CustomerRecordVisibility) => void;
  onSelect: (customer: Customer) => void;
  /** True when list is narrowed by server (search or status); empty list means "no matches" not "no customers". */
...
export default function CustomerList({
  ...
  const onRowKeyDown = (e: KeyboardEvent<HTMLTableRowElement>, c: Customer) => {
    ...
    <PremiumCard className="!p-0 flex h-full min-h-0 w-full flex-col overflow-hidden">
      ...
      <div className="shrink-0 border-b border-gray-200 px-3 py-2 dark:border-gray-700">
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
          <select
            value={statusFilter}
            onChange={(e) => onStatusFilterChange(e.target.value as 'all' | CustomerStatus)}
          ...
          <select
            ...
            onChange={(e) => onRecordVisibilityChange(e.target.value as CustomerRecordVisibility)}
            className={`${premiumInputCompact} ${premiumTypography.tableCell}`}
      ...
      <div className={`flex min-h-0 flex-1 flex-col overflow-hidden ${premiumTypography.tableCell}`}>
        ...
        {!isLoading && !error && customers.length === 0 && !filterActive && (
          <div className="p-3 text-gray-400">
            <p className={premiumTypography.helper}>No customers yet.</p>
        ...
        {!isLoading && customers.length === 0 && filterActive && (
