...
const STATUS_FILTERS: { value: 'all' | WarehouseStatus; label: string }[] = [
  { value: 'all', label: 'All' },
...
const VIS_FILTERS: { value: WarehouseRecordVisibility; label: string }[] = [
...
interface WarehouseListProps {
  ...
  statusFilter: 'all' | WarehouseStatus;
  onStatusFilterChange: (value: 'all' | WarehouseStatus) => void;
  recordVisibility: WarehouseRecordVisibility;
  onRecordVisibilityChange: (value: WarehouseRecordVisibility) => void;
  defaultOnly: boolean;
...
export default function WarehouseList({
  ...
  const onRowKeyDown = (e: KeyboardEvent<HTMLTableRowElement>, w: Warehouse) => {
    ...
    <PremiumCard className="!p-0 flex h-full min-h-0 w-full flex-col overflow-hidden">
      ...
      <div className="shrink-0 border-b border-gray-200 px-3 py-2 dark:border-gray-700">
        <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
          <select
            value={statusFilter}
            onChange={(e) => onStatusFilterChange(e.target.value as 'all' | WarehouseStatus)}
          ...
          <select
            ...
            onChange={(e) => onRecordVisibilityChange(e.target.value as WarehouseRecordVisibility)}
            className={`${premiumInputCompact} ${premiumTypography.tableCell}`}
          ...
          <select
            value={defaultOnly ? 'default_only' : 'all_sites'}
            onChange={(e) => onDefaultOnlyChange(e.target.value === 'default_only')}
            className={`${premiumInputCompact} ${premiumTypography.tableCell}`}
            aria-label="Filter by default warehouse"
          >
            <option value="all_sites">All sites</option>
            <option value="default_only">Default site only</option>
          </select>
        </div>
      </div>
      ...
      <div className={`flex min-h-0 flex-1 flex-col overflow-hidden ${premiumTypography.tableCell}`}>
        ...
        {!isLoading && !error && warehouses.length === 0 && !filterActive && (
          <div className="p-3 text-gray-400">
            <p className={premiumTypography.helper}>No warehouses found.</p>
            <p className={`mt-1 ${premiumTypography.helper}`}>
              ...
              to add a site for stock and fulfilment.
            </p>
          </div>
        )}
        {!isLoading && warehouses.length === 0 && filterActive && (
          ...
          <div className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain">
            <table className="w-full table-fixed text-left">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {warehouses.map((w) => {
                  const selected = w.id === selectedWarehouseId;
                  return (
                    <tr
                      key={w.id}
                      tabIndex={0}
                      aria-selected={selected}
                      aria-label={`${w.name}, code ${formatWarehouseCode(w.warehouse_code)}`}
                    ...
                    >
                      <td className="truncate px-2 py-1.5 text-gray-900 dark:text-white">
                        {formatWarehouseCode(w.warehouse_code)}
                      </td>
