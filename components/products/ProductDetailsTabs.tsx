...
export default function ProductDetailsTabs({ product }: ProductDetailsTabsProps) {
  ...
  const handleDeleteVariant = (id: string) => {
    setConfirmDialog({
      ...
      description: 'This removes the variant from the catalog. You can’t undo this from here.',
      onConfirm: async () => {
        try {
  ...
  const handleDeletePriceItem = (id: string) => {
    setConfirmDialog({
      title: 'Remove price list entry?',
      description: 'This product will no longer have that price on the selected list.',
      onConfirm: async () => {
        try {
            ...
            <div>
              ...
              <div className="mt-3 flex items-center gap-2">
                ...
                <p className="text-[10px] text-gray-500">
                  Saves your category choices for this product.
                </p>
              </div>
            </div>
          )}
