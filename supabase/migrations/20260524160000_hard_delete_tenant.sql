-- Irreversible purge of one tenant and all public tenant-scoped rows.
-- Callable only by service_role (API route). Never use for normal offboarding.

CREATE OR REPLACE FUNCTION public._hard_delete_tenant_rows(
  p_tenant_id uuid,
  p_table text
)
RETURNS void
...
BEGIN
  ...
  -- Optional isolated schema (drop before public purge)
  ...
  -- Cost card / logistics (children before parents)
  ...
  -- tenant_sellable_pack_levels: deleted after product_barcodes + packing_configurations (see product stack)
  ...
  -- Access control (group children first)
...
COMMENT ON FUNCTION public.hard_delete_tenant(uuid) IS
  'Permanently deletes one tenant and tenant-scoped public data. Template tenants are rejected.';
...
REVOKE ALL ON FUNCTION public._hard_delete_tenant_rows(uuid, text) FROM PUBLIC;
