...
1. [TIMEZONE_OPTIMIZATION.md](../TIMEZONE_OPTIMIZATION.md) - Usage guide for timezone caching (if present)
2. [SUPABASE_OPTIMIZATION_SUMMARY.md](../SUPABASE_OPTIMIZATION_SUMMARY.md) - Complete optimization journey
3. [TYPE_GENERATION.md](../TYPE_GENERATION.md) - Type generation setup and usage
...
// ✅ Fully typed queries
const { data } = await supabase.from('cached_timezones').select('*').order('name');
...
2. **Deploy Migration 025** to clear materialized view linter warning:
   ...
   ```bash
   # Apply the equivalent change via Supabase CLI or SQL editor using `supabase/migrations/` (timestamped files).
   ```
...
- **Timezone Caching Guide:** [TIMEZONE_OPTIMIZATION.md](../TIMEZONE_OPTIMIZATION.md) (if present)
- **Type Generation Setup:** [TYPE_GENERATION.md](../TYPE_GENERATION.md)
- **Migration files:** See `supabase/migrations/` (timestamped Supabase migrations)
