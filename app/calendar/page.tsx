...
export default function CalendarPage() {
  ...
  useEffect(() => {
    if (calendarData && todayRowRef.current && tableContainerRef.current) {
      setTimeout(() => {
        ...
        const scrollTop =
          container.scrollTop + (rowRect.top - containerRect.top) - headerHeight - 20;
    ...
    if (tenant_id) {
      ...
      if (originalEntry) {
        ...
        if (editData.bank_holiday !== originalEntry.bank_holiday) {
          changes.bank_holiday = {
            before: originalEntry.bank_holiday,
            after: editData.bank_holiday,
          };
        }
        ...
        if (Object.keys(changes).length > 0) {
          await logCalendarEventUpdated(tenant_id, `calendar-${id}`, changes, user?.id || null);
        }
      }
    }
  ...
  const saveQuickUpdate = async () => {
    ...
    if (tenant_id) {
      ...
      if (Object.keys(changes).length > 0) {
        await logCalendarEventUpdated(tenant_id, `calendar-${entry.id}`, changes, user?.id || null);
      }
    }
  ...
  const processChatQuery = (query: string): string => {
    ...
    if (lowerQuery.includes('bank holiday') || lowerQuery.includes('bank holidays')) {
      const bankHolidays = calendarData.filter(
        (entry) => entry.bank_holiday && entry.bank_holiday.trim() !== ''
      );
      if (bankHolidays.length === 0) return `No bank holidays found for ${year}.`;

      const list = bankHolidays
        .map((entry) => `• ${formatDate(entry.date)} - ${entry.bank_holiday}`)
        .join('\n');
    ...
    if (lowerQuery.includes('event') && !lowerQuery.includes('no event')) {
      ...
      const list = withEvents
        .slice(0, 10)
        .map((entry) => `• ${formatDate(entry.date)} - ${entry.events}`)
        .join('\n');
  ...
  return (
    <ProtectedRoute>
      <PageContainer module="businessCore">
        <div
          className={`sticky shrink-0 ${stickyBelowTopNavClass} z-30 -mx-1 mb-4 flex flex-col gap-3 border-b border-gray-200/70 bg-gray-50 px-1 pb-3 dark:border-gray-700 dark:bg-gray-900`}
        >
          <PremiumStickyHeader
            module="businessCore"
            sticky={false}
          ...
          {/* Statistics Panel — pins with page title + year controls */}
          {stats && (
            <div className="rounded-xl border border-gray-200 bg-gray-50/50 p-4 shadow-md ring-1 ring-black/5 dark:border-gray-700 dark:bg-gray-900/30 dark:ring-white/5">
        ...
        <div className="mb-4 flex flex-wrap gap-3 items-center bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
          ...
          <div className="flex items-center gap-2 bg-gray-50 dark:bg-gray-700/50 px-3 py-1.5 rounded-lg">
            ...
            />
            <button onClick={jumpToDate} className={premiumTertiaryButton('sm', 'standard')}>
              Go
            </button>
          </div>
            ...
            {showColumnMenu && (
              <div className="absolute right-0 mt-2 bg-white dark:bg-gray-800 rounded-lg shadow-lg p-3 z-20 min-w-[150px] border border-gray-200 dark:border-gray-700">
                {['day', 'month', 'year', 'week', 'bankHoliday', 'events', 'notes'].map((col) => (
                  <label
                    key={col}
                    className="flex items-center gap-2 py-1 cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-700 px-2 rounded"
                  >
                    <input
                      type="checkbox"
                    ...
                    <span className="text-sm capitalize">
                      {col === 'bankHoliday' ? 'Bank Holiday' : col}
                    </span>
                  </label>
                ))}
              </div>
          ...
          >
            <table className="w-full">
              ...
              >
                <tr>
                  <th
                    className="px-3 py-2 text-left text-xs font-medium text-gray-600 dark:text-gray-300 cursor-default uppercase tracking-wide w-36"
                    draggable={false}
                  >
                    Date
                  </th>
                  {!hiddenColumns.has('day') && (
                    <th
                      className="px-3 py-2 text-left text-xs font-medium text-gray-600 dark:text-gray-300 cursor-default uppercase tracking-wide w-24"
                      draggable={false}
                    >
                      Day
                    </th>
                  )}
                  {!hiddenColumns.has('month') && (
                    <th
                      className="px-3 py-2 text-left text-xs font-medium text-gray-600 dark:text-gray-300 cursor-default uppercase tracking-wide w-28"
                      draggable={false}
                    >
                      Month
                    </th>
                  ...
                  {!hiddenColumns.has('year') && (
                    <th
                      className="px-3 py-2 text-left text-xs font-medium text-gray-600 dark:text-gray-300 cursor-default uppercase tracking-wide w-20"
                      draggable={false}
                    >
                      Year
                    </th>
                  )}
                  {!hiddenColumns.has('week') && (
                    <th
                      className="px-3 py-2 text-left text-xs font-medium text-gray-600 dark:text-gray-300 cursor-default uppercase tracking-wide w-20"
                      draggable={false}
                    >
                  ...
                  {!hiddenColumns.has('bankHoliday') && (
                    <th
                      className="px-3 py-2 text-left text-xs font-medium text-gray-600 dark:text-gray-300 cursor-default uppercase tracking-wide w-48"
                      draggable={false}
                    >
                      Bank Holiday
                    </th>
                  )}
                  {!hiddenColumns.has('events') && (
                    <th
                      className="px-3 py-2 text-left text-xs font-medium text-gray-600 dark:text-gray-300 cursor-default uppercase tracking-wide w-48"
                      draggable={false}
                    >
                  ...
                  {!hiddenColumns.has('notes') && (
                    <th
                      className="px-3 py-2 text-left text-xs font-medium text-gray-600 dark:text-gray-300 cursor-default uppercase tracking-wide"
                      draggable={false}
                    >
                      Notes
                    </th>
                  ...
                  <th
                    className="py-2 text-center text-xs font-medium text-gray-600 dark:text-gray-300 cursor-default w-16"
                    draggable={false}
                  >
              ...
              <tbody>
                {filteredData &&
                  filteredData.map((entry: CalendarEntry, index: number) => {
                    ...
                    return (
                      ...
                      >
                        <td
                          className={`${densityClasses[density]} text-xs text-gray-900 dark:text-gray-100`}
                        >
                          {formatDate(entry.date)}
                        </td>
                        {!hiddenColumns.has('day') && (
                          ...
                          <td className={`${densityClasses[density]} text-xs`}>
                            {isEditing ? (
                              <input
                                ...
                                onChange={(e) =>
                                  setEditData({ ...editData, bank_holiday: e.target.value })
                                }
                          ...
                          <td className={`${densityClasses[density]} text-xs`}>
                            {isEditing ? (
                              <input
                                ...
                                onChange={(e) =>
                                  setEditData({ ...editData, events: e.target.value })
                                }
                                className="w-full px-2 py-1 border rounded dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                              />
                            ) : (
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setQuickUpdateData({ ...quickUpdateData, bank_holiday: e.target.value })
                    }
                    className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setQuickUpdateData({ ...quickUpdateData, events: e.target.value })
                    }
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setQuickUpdateData({ ...quickUpdateData, notes: e.target.value })
                    }
                    className="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-700 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all resize-none"
              ...
              <div className="p-6 space-y-4">
                ...
                <div className="bg-blue-50 dark:bg-blue-900/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                  <p className="text-sm text-blue-900 dark:text-blue-100">
                    This will fetch {year} bank holidays for the selected country and populate them
                    in your calendar.
                  </p>
                </div>
